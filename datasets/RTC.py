...
class RTC(Dataset):
    def __init__(
        ...
        dataset_type = "train" if train else "test_with_wheats"
    ...
    def __getitem__(self, index):
        ...
        npy_path = img_path.replace('images/', 'img_IN-norm_numpy/').replace('.jpg', '.npy')
...
def build_rtc(image_set, args):
    ...
    if image_set == 'train':
        train_set = RTC(data_root, train=True, global_crop_ratio=args.global_crop_ratio, transform=None, patch_size=args.patch_size, flip=True, total_steps=args.epochs)
        return train_set
    elif image_set == 'val':
        val_set = RTC(data_root, train=False, transform=None, patch_size=args.patch_size)
        return val_set
    else:
...
if __name__ == "__main__":
    ...
    plt.plot(dataset.prob_schedule)
    plt.savefig('./test_cosprob.png')
    for i in range(10):
        st = time.time()
