...
def get_shape(val: object) -> typing.List[int]:
    ...
    if val.isCompleteTensor():  # pyre-ignore
        ...
        return [1]
    elif val.type().kind() == 'StringType':  # maybe constant
        return []  
    else:
        raise ValueError()
...
def conv_flop_jit(
    ...
    # assert len(inputs) == 12, f'the length of node.inputs should be 12, but get {len(inputs)}'
    # if len(inputs) != 12:
    #     print(f'the length of node.inputs should be 12, but get {len(inputs)}')
    x, w = inputs[:2]
...
def matmul_flop_jit(
    ...
    batch_dim0 = input_shapes[0][:-2]
    batch_dim1 = input_shapes[1][:-2]
    assert batch_dim0 == batch_dim1
    m1_dim, m2_dim = input_shapes[1][-2:]
    flop = m1_dim * m2_dim * batch_dim0[0] * batch_dim0[1]
